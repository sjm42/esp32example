        ...
        loop {
            match read_temperature(one_wire_bus, device) {
                Ok(temperature) => {
                    ...
                    info!("Got measurement, retry#{retries}: {m:?}");
                    meas.push(m);
                    break;
                }
                Err(e) => {
...
pub async fn poll_sensors(state: Arc<Pin<Box<MyState>>>) -> anyhow::Result<()> {
    ...
    let poll_delay = state.config.sensor_poll_delay;
    let max_retry = state.config.sensor_retries;
    loop {
        if !*state.ntp_ok.read().await
            && Utc::now().year() > 2020
            && ntp.get_sync_status() == sntp::SyncStatus::Completed
        {
...

        {
            ...
            data.last_update = if *state.ntp_ok.read().await {
                now.to_rfc3339()
            } else {
                format!("{} (time not synced)", now.timestamp())
            };
